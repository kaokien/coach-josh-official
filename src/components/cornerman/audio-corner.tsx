    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-1 space-y-4">
        <div className="flex items-center justify-between border-b-2 border-black pb-2">
          <h3 className="font-display text-2xl uppercase text-[#0F172A]">Audio Workouts</h3>
          <span className="font-mono text-sm text-[#0F172A]/60">{AUDIO_WORKOUTS.length} sessions</span>
        </div>
        ...
        <div className="flex flex-nowrap gap-1.5 overflow-x-auto pb-2 scrollbar-hide">
          {(['All', 'Technique', 'Conditioning', 'Mindset'] as CategoryFilter[]).map((cat) => (
            <button
              ...
              className={`flex-shrink-0 px-2 py-1 text-[10px] font-bold uppercase tracking-wide border-2 border-[#0F172A] transition-all ${categoryFilter === cat
        ...
        {activeWorkout && (
          <div className="lg:hidden bg-[#0F172A] border-2 border-black p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
            ...
            <div className="flex items-center justify-center gap-4">
              ...
              <button
                onClick={togglePlay}
                className="p-3 border-2 border-white bg-[#DC2626] text-white hover:bg-[#B01C33] transition-colors"
              >
            ...
            {filteredWorkouts.map((workout, index) => (
              <motion.button
                ...
                className={`w-full text-left p-4 border-2 border-black transition-all relative ${activeWorkout?.id === workout.id
                  ...
                  : 'bg-white hover:bg-[#FFFFFF] shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]'
                  }`}
              >
                {/* New Badge */}
                {workout.isNew && (
                  <div className="absolute -top-2 -right-2 bg-[#DC2626] text-white text-[10px] font-bold px-2 py-0.5 border border-black flex items-center gap-1">
                    <Sparkles size={10} />
                    NEW
                  </div>
                )}
                ...
                <div className="flex justify-between items-start mb-1 pr-6">
                  <span className={`text-xs font-bold px-2 py-0.5 border border-black uppercase ${activeWorkout?.id === workout.id ? 'bg-white text-[#0F172A]' : workout.color + ' text-white'
                    }`}>
                    {workout.level}
                  </span>
                ...
                <div className="mt-2 flex items-center gap-2">
                  <span className={`text-[10px] font-mono uppercase tracking-wider ${activeWorkout?.id === workout.id ? 'text-white/50' : 'text-[#0F172A]/50'}`}>
            ...
            {activeWorkout ? (
              <div className="relative z-10 w-full max-w-2xl mx-auto">
                ...
                <div className="flex flex-col items-center text-center mb-8">
                  <span className="text-sm font-black tracking-[0.2em] uppercase text-[#0F172A]/50 mb-3">
                    {activeWorkout.category} • {activeWorkout.subcategory}
                  </span>
                  <h2 className="font-display text-3xl md:text-4xl uppercase mb-4 text-[#0F172A] leading-none">
                    {activeWorkout.title}
                  </h2>
                  <div className="flex items-center justify-center gap-4 text-xs md:text-sm font-bold font-mono border-2 border-[#0F172A] px-4 py-2 bg-[#FFFFFF] uppercase">
                ...
                <div className="mb-8">
                  <div className="relative">
                    {/* Track background */}
                    <div className="h-3 bg-[#0F172A]/10 border border-[#0F172A]/20 rounded-none">
                      {/* Progress fill */}
                      <div
                        className={`h-full ${activeWorkout.color} transition-all duration-100`}
                        style={{ width: `${progressPercentage}%` }}
                ...
                <div className="flex items-center justify-center gap-6 mb-8">
                  <button
                    onClick={() => { if (audioRef.current) audioRef.current.currentTime -= 10 }}
                    className="p-4 border-2 border-[#0F172A] bg-white hover:bg-[#FFFFFF] transition-colors shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-x-[2px] active:translate-y-[2px]"
                    aria-label="Rewind 10s"
                  >
                    <Rewind size={24} />
                  </button>
                  <button
                    onClick={togglePlay}
                    className="p-6 border-2 border-[#0F172A] bg-[#DC2626] text-white hover:bg-[#B01C33] transition-colors shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-x-[2px] active:translate-y-[2px]"
                  >
                ...
                <div className="text-center border-t-2 border-[#0F172A]/10 pt-6">
                  <p className="text-sm text-[#0F172A]/60 italic font-mono">&quot;Listen to the instructions. Visualize the opponent. Don&apos;t just go through the motions.&quot;</p>
                </div>
              </div>
            ) : (
